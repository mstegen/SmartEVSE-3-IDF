...
----------+            |             |  +-----------------------------
          ...
          +-----------------------------+
*/
#define ONEWIRE_SLOT_START_DURATION             6  // bit start pulse duration (was 2; SmartEVSE: the iButton/RFID
                                                   // reader responds to a read '0' ~6us after the slot's falling
                                                   // edge. A 2us start pulse releases the bus before the reader
                                                   // pulls it low, splitting every '0' bit into two RMT symbols and
                                                   // desyncing the 1-symbol-per-bit decoder. 6us keeps the master
                                                   // low until the reader's low overlaps, matching the proven
                                                   // SmartEVSE-3.5 driver. Still <15us so read-'1' decodes correctly.
...
static esp_err_t onewire_bus_rmt_read_bytes(onewire_bus_handle_t bus, uint8_t *rx_buf, size_t rx_buf_size)
{
    ...
    if (max_bytes_per_chunk > rx_buf_size) {
        max_bytes_per_chunk = rx_buf_size;
    }
