...
#elif MO_USE_FILEAPI == ESPIDF_LITTLEFS
...
#ifndef MO_PARTITION_LABEL
#define MO_PARTITION_LABEL "littlefs"
#endif

namespace MicroOcpp {

class EspIdfLittlefsFileAdapter : public FileAdapter, public MemoryManaged {
    FILE *file {nullptr};
public:
    EspIdfLittlefsFileAdapter(FILE *file) : MemoryManaged("Filesystem"), file(file) {}

    ~EspIdfLittlefsFileAdapter() {
        fclose(file);
    }
...
class EspIdfLittlefsFilesystemAdapter : public FilesystemAdapter, public MemoryManaged {
public:
    FilesystemOpt config;

    void (* onDestruct)(void*) = nullptr;
public:
    EspIdfLittlefsFilesystemAdapter(FilesystemOpt config, void (* onDestruct)(void*) = nullptr) : MemoryManaged("Filesystem"), config(config), onDestruct(onDestruct) { }

    ~EspIdfLittlefsFilesystemAdapter() {
        if (config.mustMount()) {
            esp_vfs_littlefs_unregister(MO_PARTITION_LABEL);
            MO_DBG_DEBUG("LittleFS unmounted");
        }
    ...
    std::unique_ptr<FileAdapter> open(const char *fn, const char *mode) override {
        auto file = fopen(fn, mode);
        if (file) {
            return std::unique_ptr<FileAdapter>(new EspIdfLittlefsFileAdapter(std::move(file)));
        } else {
            MO_DBG_DEBUG("Failed to open file path %s", fn);
            return nullptr;
        }
...
std::shared_ptr<FilesystemAdapter> makeDefaultFilesystemAdapter(FilesystemOpt config) {
    ...
    if (!config.accessAllowed()) {
        MO_DBG_DEBUG("Access to ESP-IDF LittleFS not allowed by config");
        return nullptr;
    }
    ...
    if (config.mustMount()) {
        ...
        if (ret == ESP_OK) {
            mounted = true;
            MO_DBG_DEBUG("LittleFS mounted");
        } else {
            if (ret == ESP_FAIL) {
                MO_DBG_ERR("Failed to mount or format filesystem");
            } else if (ret == ESP_ERR_NOT_FOUND) {
                MO_DBG_ERR("Failed to find LittleFS partition");
            } else {
                MO_DBG_ERR("Failed to initialize LittleFS (%s)", esp_err_to_name(ret));
            }
        }
    }

    if (mounted) {
        auto fs = std::shared_ptr<FilesystemAdapter>(new EspIdfLittlefsFilesystemAdapter(config, resetFilesystemCache), std::default_delete<FilesystemAdapter>(), makeAllocator<FilesystemAdapter>("Filesystem"));
