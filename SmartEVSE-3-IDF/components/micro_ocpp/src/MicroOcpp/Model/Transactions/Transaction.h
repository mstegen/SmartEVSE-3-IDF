...
/* General Tx defs - C-compatible enum that lives in the GLOBAL
 * namespace for backward compatibility with the original MicroOcpp
 * ABI (MicroOcpp.cpp / Connector.h reference the unqualified
 * `TxNotification` and `TxNotification_Authorized` symbols). The
 * C++ aliases below give the v3 firmware (esp32.cpp, glcd.cpp) the
 * `MicroOcpp::TxNotification::Authorized` scoped access it expects. */
#ifdef __cplusplus
extern "C" {
#endif //__cplusplus
...
#ifdef __cplusplus
/* v3 firmware (esp32.cpp, glcd.cpp) references the enum as
 * `MicroOcpp::TxNotification::Authorized`. Internally, MicroOcpp
 * uses the global C-style typedef `::TxNotification` and its
 * enumerator `::TxNotification_Authorized`. To bridge the two:
 *
 *   1. `MicroOcpp::TxNotification` is a using-alias for the global
 *      `::TxNotification` C-enum. The MicroOcpp signatures in
 *      MicroOcpp.h (`std::function<void(Transaction*, TxNotification)>`)
 *      resolve `TxNotification` in their parameter list at global
 *      scope (the parameter is unqualified), and the v3 firmware's
 *      callback signature `void(MicroOcpp::Transaction*, MicroOcpp::TxNotification event)`
 *      binds via the alias.
 *
 *   2. To make `MicroOcpp::TxNotification::Authorized` work, we
 *      add the enumerator names as namespace-scope constants inside
 *      the MicroOcpp namespace. With C++ name lookup, when the v3
 *      firmware writes `MicroOcpp::TxNotification::Authorized`,
 *      the compiler first looks for `Authorized` as a member of
 *      the type `MicroOcpp::TxNotification` (which is the global
 *      C-enum). It doesn't find it there, so it falls back to
 *      looking up `MicroOcpp::Authorized` (a namespace-scope
 *      constant we define below). The result is the same
 *      enumerator value. */
namespace MicroOcpp {
    using TxNotification = ::TxNotification;
    constexpr TxNotification Authorized               = ::TxNotification_Authorized;
    constexpr TxNotification AuthorizationRejected   = ::TxNotification_AuthorizationRejected;
    constexpr TxNotification DeAuthorized             = ::TxNotification_DeAuthorized;
    constexpr TxNotification AuthorizationTimeout    = ::TxNotification_AuthorizationTimeout;
    constexpr TxNotification ReservationConflict      = ::TxNotification_ReservationConflict;
    constexpr TxNotification ConnectionTimeout        = ::TxNotification_ConnectionTimeout;
    constexpr TxNotification RemoteStart             = ::TxNotification_RemoteStart;
    constexpr TxNotification RemoteStop              = ::TxNotification_RemoteStop;
    constexpr TxNotification StartTx                 = ::TxNotification_StartTx;
    constexpr TxNotification StopTx                  = ::TxNotification_StopTx;
    constexpr TxNotification UNDEFINED                = ::TxNotification_UNDEFINED;
}
#endif
